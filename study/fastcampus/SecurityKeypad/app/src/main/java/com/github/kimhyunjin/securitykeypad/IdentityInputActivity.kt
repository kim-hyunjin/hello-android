...
class IdentityInputActivity : AppCompatActivity() {
    ...
    private fun initView() {
        ...
        with(binding) {
            ...
            birthdayEdit.setEditorActionListener(EditorInfo.IME_ACTION_DONE) {
                if (birthdayEdit.length() > 7 && validBirthday()) {
            ...
            phoneEdit.setEditorActionListener(EditorInfo.IME_ACTION_DONE) {
                if (phoneEdit.length() > 9 && validPhone()) {
                    confirmButton.isVisible = true
                    phoneLayout.error = null
                } else {
                    confirmButton.isVisible = false
    ...
    private fun validBirthday() =
        !binding.birthdayEdit.text.isNullOrBlank() && REGEX_BIRTHDAY.toRegex()
